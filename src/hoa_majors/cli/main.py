...
def main():
    ...
    search_parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="数据存储目录")

    # audit
    audit_parser = subparsers.add_parser("audit", help="审计课程名称与代码的冲突")
    audit_parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="数据存储目录")
    audit_parser.add_argument("--output", type=Path, help="保存冲突报告的路径")

    # plans
    plans_parser = subparsers.add_parser("plans", help="列出所有已抓取的培养方案")
    plans_parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="数据存储目录")

    # courses
    courses_parser = subparsers.add_parser("courses", help="列出特定培养方案的所有课程")
    courses_parser.add_argument("plan_id", help="培养方案 ID (fah)")
    courses_parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="数据存储目录")
